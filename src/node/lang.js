...
function getLanguageData(req, res, cb){
  ...
  var langPathLocal = req.options.langPathLocal || null;
  var url = req.workerOptions.langPath + langfile;

  fs.readFile(path.join(langPathLocal, lang + '.traineddata'), function (err, data) {
    ...
    http.get(url, function(stream){
      ...
      stream.pipe(gunzip).pipe(fs.createWriteStream(path.join(langPathLocal, lang + '.traineddata')));
